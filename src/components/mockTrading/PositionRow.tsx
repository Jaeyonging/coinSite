...
const PositionRow = ({ position, coin, krCoin, onCoinClick, isMobile }: PositionRowProps) => {
  ...
  return (
    <tr
      ...
      className="cursor-pointer border-b border-gray-200 transition-colors duration-200 ease-in-out md:hover:bg-gray-50"
    >
      <CoinNameCell
        position={position}
        coin={coin}
        ...
        isMobile={isMobile}
      />
      <td className="hidden md:table-cell py-3.5 px-3 text-right text-sm font-semibold">
        {formatPrice(position.avgPrice * position.amount)}원
      </td>
      <td className="hidden md:table-cell py-3.5 px-3 text-right text-sm font-medium">
        {position.amount.toLocaleString()}개
      </td>
      <td className="hidden md:table-cell py-3.5 px-3 text-right text-sm">
      ...
      <td className="py-1.5 px-0 md:py-3.5 md:px-3 text-[9px] md:text-sm text-right font-semibold">
        ...
        <span className="hidden md:inline">{formatPrice(krCoin.krwprice)}원</span>
      </td>
      <PriceCell
      ...
      />
      {!isMobile && (
        <td className="py-3.5 px-3 text-center text-sm text-gray-500">
